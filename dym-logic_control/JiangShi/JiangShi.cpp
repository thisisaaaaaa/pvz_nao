	...
	switch (type)
	{
		case Type::normal:
			...
			for (int i = 0; i < 22; i++)
			{
				...
				loadimage(&walk[i], _T(filename),width,height,true);
			}
			break;
		case Type::luzhang:
			health = 150;
...
void JiangShi::draw(int delta,int change_time)
{
	...
	{
		int timer = 0,index = 0; // 每30毫秒切换一次图片
		timer += delta; // 增加计时器
		...
		putimage(position.x, position.y, &walk[index]); // 绘制行走图片
	}
}

State JiangShi::get_state() 
{
	return state;
}

vector<double> JiangShi::get_position() 
{
